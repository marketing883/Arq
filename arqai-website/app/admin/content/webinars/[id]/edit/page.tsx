...
export default function EditWebinarPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    registration_url: "",
    recording_url: "",
    status: "upcoming" as "upcoming" | "live" | "on-demand" | "past" | "draft",
    featured: false,
  ...
  useEffect(() => {
    async function fetchWebinar() {
      try {
        const response = await fetch(`/api/admin/content/webinars/${webinarId}`);
        if (response.ok) {
          const data = await response.json();
          if (data.item) {
            // Parse webinar_date into date and time
            let webinarDate = "";
            let webinarTime = "";
            if (data.item.webinar_date) {
              const date = new Date(data.item.webinar_date);
              webinarDate = date.toISOString().split("T")[0];
              webinarTime = date.toTimeString().slice(0, 5);
            }
            ...
            setFormData({
              ...
              banner_image: data.item.banner_image || "",
              webinar_date: webinarDate,
              webinar_time: webinarTime,
              duration: data.item.duration || 60,
              timezone: data.item.timezone || "America/New_York",
              presenters: data.item.presenters?.length > 0 ? data.item.presenters : [{ name: "", title: "", company: "", photo: "" }],
              learning_points: data.item.learning_points?.length > 0 ? data.item.learning_points : [""],
              registration_url: data.item.registration_url || "",
              recording_url: data.item.recording_url || "",
              status: data.item.status || "upcoming",
  ...
  const handleSubmit = async (publishStatus: "draft" | "upcoming" | "on-demand") => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || "Failed to update webinar");
      }
    } catch (error) {
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <span className="font-semibold text-slate-900">Edit Webinar</span>
            </div>
          ...
          {formData.banner_image ? (
            ...
            >
              <div className={`border-2 border-dashed rounded-lg p-12 text-center transition-colors ${
                isDraggingBanner ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:border-blue-400"
              }`}>
                {isUploadingBanner ? (
                  ...
                  <>
                    ...
                    <p className="text-slate-600 font-medium">{isDraggingBanner ? "Drop image here" : "Click or drag to upload banner image"}</p>
