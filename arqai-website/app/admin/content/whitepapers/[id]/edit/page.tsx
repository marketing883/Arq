...
export default function EditWhitepaperPage() {
  ...
  const whitepaperId = params.id as string;
  ...
  useEffect(() => {
    async function fetchWhitepaper() {
      try {
        const response = await fetch(`/api/admin/content/whitepapers/${whitepaperId}`);
        if (response.ok) {
  ...
  const handleSubmit = async (publishStatus: "draft" | "published") => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || "Failed to update whitepaper");
      }
    } catch (error) {
      console.error("Submit error:", error);
      alert("Failed to update whitepaper");
    } finally {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <span className="font-semibold text-slate-900">Edit Whitepaper</span>
            </div>
          ...
          {formData.cover_image ? (
            ...
            >
              <div className={`border-2 border-dashed rounded-lg p-12 text-center transition-colors ${
                isDraggingCover ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:border-blue-400"
              }`}>
                {isUploadingCover ? (
                  ...
                  <>
                    ...
                    <p className="text-slate-600 font-medium">{isDraggingCover ? "Drop image here" : "Click or drag to upload cover image"}</p>
          ...
          {formData.file_url ? (
            ...
            >
              <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
                isDraggingFile ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:border-blue-400"
              }`}>
                {isUploadingFile ? (
                  <div className="w-10 h-10 border-3 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto" />
                ) : (
                  <>
                    ...
                    <p className="text-slate-600 font-medium">{isDraggingFile ? "Drop PDF here" : "Click or drag to upload PDF"}</p>
