...
export default function ContentPage() {
  ...
  const [activeTab, setActiveTab] = useState<ContentType>(typeParam || "blog");
  const [items, setItems] = useState<ContentItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Update active tab when URL param changes
  useEffect(() => {
    if (typeParam && ["blog", "case-studies", "whitepapers", "webinars"].includes(typeParam)) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <AdminHeader
        ...
        subtitle="Create and manage blog posts, case studies, and resources"
      />
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-5">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex flex-col items-start gap-0.5 p-3 rounded text-left transition-all ${
                activeTab === tab.id
            ...
            >
              ...
              <span className={`text-[10px] ${activeTab === tab.id ? "text-slate-400" : "text-slate-400"}`}>
                {tab.description}
              </span>
            </button>
          ))}
        ...
        <div className="bg-white rounded-md border border-slate-200 overflow-hidden">
          ...
          <div className="p-3 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-medium text-slate-900 capitalize">{activeTab.replace("-", " ")}</h2>
              <p className="text-[10px] text-slate-500">{items.length} item{items.length !== 1 ? "s" : ""}</p>
            </div>
            <button
              onClick={handleAddNew}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 text-white text-xs font-medium rounded hover:bg-slate-800 transition-colors"
            >
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <div className="w-12 h-12 mx-auto mb-3 rounded-md bg-slate-100 flex items-center justify-center">
                <svg className="w-6 h-6 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <p className="text-sm text-slate-600 font-medium">No {activeTab.replace("-", " ")} yet</p>
              <p className="text-xs text-slate-400 mt-1 mb-4">Get started by creating your first item</p>
              <button
                onClick={handleAddNew}
              ...
              >
                ...
                Create First {activeTab === "blog" ? "Post" : activeTab === "case-studies" ? "Case Study" : activeTab === "whitepapers" ? "Whitepaper" : "Webinar"}
              </button>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h5 className="text-xs font-medium text-slate-900 truncate">{item.title}</h5>
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${getStatusColor(item.status)}`}>
                          {item.status}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 text-[10px] text-slate-500">
                        ...
                        <span className="text-slate-300">•</span>
                        <span className="text-slate-400 truncate max-w-[150px]">/{item.slug}</span>
                      </div>
                    </div>
        ...
        <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {tabs.map((tab) => (
            <Link
              ...
              className="flex items-center gap-2.5 p-3 bg-white border border-slate-200 rounded hover:border-slate-300 hover:shadow-sm transition-all group"
            >
              <div className="w-8 h-8 rounded bg-slate-100 group-hover:bg-slate-200 flex items-center justify-center transition-colors">
                <span className="text-slate-500 group-hover:text-slate-700 transition-colors w-4 h-4">
                  {tab.icon}
                </span>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-900 group-hover:text-slate-700 transition-colors">
                  New {tab.id === "blog" ? "Post" : tab.id === "case-studies" ? "Case Study" : tab.id === "whitepapers" ? "Whitepaper" : "Webinar"}
                </p>
                <p className="text-[10px] text-slate-400">Create new</p>
              </div>
            </Link>
          ))}
        </div>
