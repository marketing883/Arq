...
function getPriorityTier(lead: LeadData): { tier: string; color: string; bgColor: string; action: string } {
  ...
  if (buy_intent_score >= 70 || urgency === "immediate" || qualification_status === "qualified") {
    return { tier: "Priority 1", color: "text-red-700", bgColor: "bg-red-100", action: "Contact within 1 hour" };
  }
  if (buy_intent_score >= 40 || company_size === "enterprise" || urgency === "high") {
    return { tier: "Priority 2", color: "text-amber-700", bgColor: "bg-amber-100", action: "Contact within 24 hours" };
  }
  if (buy_intent_score >= 20 || qualification_status === "nurture") {
    return { tier: "Priority 3", color: "text-slate-700", bgColor: "bg-slate-200", action: "Add to nurture sequence" };
  }
  return { tier: "Priority 4", color: "text-slate-500", bgColor: "bg-slate-100", action: "Monitor activity" };
}
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-6">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mb-6 p-5 bg-slate-900 rounded-xl text-white"
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-2">AI Insights</h2>
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-emerald-600 text-sm font-medium">Qualified</span>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-slate-600 text-sm font-medium">Last 24h</span>
                <span className="p-2 bg-slate-100 rounded-lg">
              ...
              <p className="text-3xl font-bold text-slate-900">{stats.recentLeads}</p>
              <p className="text-xs text-slate-500 mt-1">New conversations</p>
            </motion.div>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1 flex flex-wrap gap-3">
            <select
              ...
              className="px-4 py-2.5 bg-white rounded-lg border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
            >
              ...
              {leads.map((lead, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white rounded-xl p-5 border border-slate-200 hover:border-slate-300 hover:shadow-md cursor-pointer transition-all group"
                  >
                    {/* Priority Badge */}
                    <div className="flex items-start justify-between mb-4">
                      <div className={`px-3 py-1 ${priority.bgColor} rounded-lg`}>
                        <span className={`text-xs font-semibold ${priority.color}`}>{priority.tier}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-medium capitalize ${
                        lead.intelligence.intent_category === "hot" ? "bg-red-50 text-red-700" :
                        lead.intelligence.intent_category === "warm" ? "bg-amber-50 text-amber-700" :
                  ...
                  {leads.map((lead) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 ${priority.bgColor} rounded-md text-xs font-medium ${priority.color}`}>
                            {priority.tier}
                          </span>
                        </td>
              ...
              >
                ...
                <div className="p-6 bg-slate-900 text-white">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className={`inline-flex px-3 py-1 ${getPriorityTier(selectedLead).bgColor} rounded-md text-xs font-semibold ${getPriorityTier(selectedLead).color} mb-3`}>
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-6">
                    <div className="bg-white/10 rounded-lg p-3">
                      <p className="text-slate-400 text-xs">Intent Score</p>
                ...
                <div className="p-6 overflow-y-auto max-h-[50vh] space-y-6">
                      ...
                      <div className="space-y-2">
                        {Array.from(new Map(selectedLead.intelligence.behavioral_signals.map(s => [s.type + s.content.substring(0, 30), s])).values())
                          ...
                          <div key={idx} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                            <span className="px-2.5 py-1 bg-slate-200 text-slate-700 text-xs font-medium rounded-md whitespace-nowrap">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedLead.intelligence.company_research.compliance_requirements.map((req) => (
                                <span key={req} className="px-3 py-1 bg-slate-200 text-slate-700 text-sm font-medium rounded-md">
                                  {req}
                                </span>
                              ))}
                            </div>
                  ...
                  <div>
                    ...
                    <div className="p-4 bg-slate-100 rounded-lg border border-slate-200">
                      <p className="text-slate-800 font-medium">{getPriorityTier(selectedLead).action}</p>
                      {selectedLead.user.email && (
                        <a
                          href={`mailto:${selectedLead.user.email}?subject=Following up on your ArqAI inquiry`}
                          className="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors"
                        >
