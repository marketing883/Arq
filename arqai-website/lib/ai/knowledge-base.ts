...
## Accelerators
When the same operating pattern appears often enough, we offer an accelerator to shorten the path from discovery to production. Accelerators are productized, named workflow patterns — not generic apps. Each is a reusable starting point that is adapted to the customer's data, policies, systems, approval paths, and operating metric through services and the ArqAI operating fabric. The portfolio is ten production-ready accelerators, all governed from day one — four vertical (industry-specific) and six horizontal (cross-industry) — each with a page at /accelerators/{id}:

Vertical:
- **ArqFWA** (flagship) — Payment integrity and fraud intelligence for healthcare payers, TPAs, and PBMs. Correlates claims, provider, policy, and member signals to surface high-value FWA cases with an evidence trail. Design targets: 30%+ more high-value cases surfaced, 2x faster triage, 100% decision evidence captured. Real result: one Blind Spot Assessment surfaced ~$3.2M in undetected waste at a mid-size TPA and cut manual review time ~70%.
...
- **ArqLogistics** — Supply chain and vendor risk intelligence. Dependency-graph monitoring, external signal fusion, and exposure quantification; designed to surface risk signals 30-60 days before operational impact, including tier-2/tier-3 suppliers.
- **ArqBanker** — AI-native banking operations: underwriting, KYC onboarding, AML/fraud surveillance, and regulatory reporting with explainable reasoning. Design targets: faster underwriting, sharply reduced KYC manual review, higher fraud-detection precision.

Horizontal:
- **ArqForecast** — Demand, inventory, and cash flow forecasting from a 20+ model ensemble; designed to deploy in 15-30 days and improve accuracy over spreadsheet baselines.
- **ArqSupport** — Agentic L1/L2/L3 ticket management: intelligent triage, knowledge-base auto-resolution (designed for 40-60% of L1, coverage-dependent), and proactive SLA monitoring for IT, HR, finance, and shared services.
- **ArqDataQ** — Multi-agent data quality: real-time pipeline monitoring, lineage-aware impact assessment, and autonomous remediation of known issue patterns.
...
export const PAGE_CONTEXT_PROMPTS: Record<string, string> = {
  ...
  "/accelerators": "The user is on accelerators. There are ten productized patterns, all governed from day one — four vertical (ArqFWA, ArqLoyalty, ArqLogistics, ArqBanker) and six horizontal (ArqForecast, ArqSupport, ArqDataQ, ArqVantage, ArqSecOps, ArqEye). Help them understand which one maps to their operation, or whether a custom build is the right path.",
